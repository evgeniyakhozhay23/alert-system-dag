...
def check_sigma(df_2, metric, a=3, n=5):
    df_2['rolling_mean'] = df_2[metric].shift(1).rolling(n, center=True, min_periods=1).mean()
    df_2['sigma'] = df_2[metric].shift(1).rolling(n).std()
    ...
    def run_alerts(df_total, chat=None):
        ...
        for metric in metrics:
            ...
            if df[metric].iloc[-1] < df['low'].iloc[-1]:
                ...
                text = "Нарушение по верхней границе"
            ...
            if df_2[metric].iloc[-1] < df_2['low_level'].iloc[-1]:
                ...
                text_sigma = "Нарушение по верхней границе"
